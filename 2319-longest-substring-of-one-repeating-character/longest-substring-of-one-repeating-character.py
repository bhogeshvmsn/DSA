class ST:
    ...
    def merge(self,left,right):
        lpref,lsuf,lbest,llchar,lrchar,llen=left
        ...
        if lrchar==rlchar:
            ...
            if lpref==llen:pref=llen+rpref
            if rsuf==rlen:suf=rlen+lsuf
        return (pref,suf,best,llchar,rrchar,llen+rlen)
    ...
    def update(self,l,r,node,idx,val):
        ...
        mid=l+(r-l)//2
        if idx<=mid: self.update(l,mid,node*2,idx,val)
        else: self.update(mid+1,r,node*2+1,idx,val)
